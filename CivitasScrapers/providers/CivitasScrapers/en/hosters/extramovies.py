...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        if 1:#try:
            ...
            hostDict = set(hostDict)
            for item_url, name ,release in match:
                release = release.strip()
                if year == release:
                    if year in release:
                        self.get_source(item_url,title,year,'','',start_time, hostDict)
            return self.sources2


    def get_source(self,item_url,title,year,season,episode,start_time, hostDict):
        if 1:#try:
            rez = item_url
            if '1080' in rez:
                res = '1080p'
            ...
            for link in Regex:
                ...
                try:
                    if not any(i in link for i in hostDict):
                        continue
                except:
                    continue
                if 'Www' not in host:
